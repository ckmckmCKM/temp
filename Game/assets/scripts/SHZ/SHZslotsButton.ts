...
export default class clikeButton extends cc.Component {
    ...
    ListSetLayerCall(){
        ...
        if(window.slotsScene.setLayer != null){
            ...
            window.slotsScene.setLayer.node.active = false;
    ...
    monitor_Game_ButtonState(index){  
        for(let i = 0; i < 3; i++){
            if(index == i ){ 
                ...
                this.Button[i].node.getComponent(cc.Button).interactable = true;
            }else{
    ...
    setSlotsSceneBetsTextUpdata(){
        ...
        let totalbets = window.slotsScene.isGameLine * window.slotsScene.isGameBetsAny[window.slotsScene.isGameBets] * 0.01;
        window.slotsScene.gameText[2].string = "￥" + totalbets.toFixed(2);
    }
    // 刷新金币
    setUserGoldUpdata(){
    ...
    textButtonTips(str){   
        ...
        window.slotsScene.isFreeShowSprite[0].getChildByName("Label").getComponent(cc.Label).string = str;
        window.slotsScene.isFreeShowSprite[0].runAction(cc.sequence(fadeIn,time,fadeOut))
    }
}
