...
export default class runLogic extends cc.Component {
    ...
    start() {  
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let num3 = Math.ceil(Math.random()*9)-1; 
    ...
    runPanleBeginFunc(){ 
        ...
        if(this.loopAudio == null){
            this.loopAudio = window.audioMng.playEffect(window.slotsScene.audioSource[4],true); 
        }
    ...
    runPanleHaveFunc(rollnode,line){
        ...
        let move2 = cc.moveBy(this.rollTimeState.RollAllBeginTime ,cc.v2(0,-620))//.easing(cc.easeExponentialIn());
    ...
    rollFunc(rollnode,rollNumber,line,timeSub) { 
        if( rollNumber > window.slotsScene.roolConfig.isRollModeNormalCount[line] || (window.slotsScene.isStop == true && rollNumber >= 2) ){
            ...
            if(rollnode.y > 450 && rollnode.y < 600 || rollnode.y < -250){
                ...
                rollnode.y = 0 ;
                moveto = cc.moveTo(timeSub,cc.v2(rollnode.x,-580)); 
            }
    ...
    runPanleEndFunc(rollnode,line){
        ...
        if(rollnode.y > 450 && rollnode.y < 600 || rollnode.y < -250){ 
             ...
             let num4 = Math.ceil(Math.random()*9)-1;  
        ...
        let moveto1 = cc.moveBy(endTime,cc.v2(0,-680));
