class common {
    ...
    static chip_fiy(begin_node, end_node_move, sprite,value, self,time) {
        ...
        let move = cc.moveTo(time, pos.x + Math.ceil(Math.random() * 350), pos.y + Math.ceil(Math.random() * 100)); 
    ...
    static chip_operation(gold_num) {          
        if (gold_num >= common.gold_lv[2]) {
            ...
            if (lv1_mo >= common.gold_lv[1]) {
               ...
               // cc.log('出现的情况----' + gold_num);
            }
        }
        else if (gold_num < common.gold_lv[2]) {
            //cc.log('gold_lv123123');
    ...
    static banker_fiy(end_node,pox,poy, sprite, self,time) {
        let chip_new = new cc.Node("chip sprite");  //创建新的node3
        chip_new.zIndex = 4
    ...
    static card_cun(num){
        // for (let i = 0; i < 5; i++) {
        //     if (common.card_game_end[i] != null) {
        //         
        //     }
        // }  
        common.card_game_end[num] = []      
    }
    ...
    static card_fiy(begin_node, end_node, sprite, seat, index, people_num, self,time) {
        ...
        let player_have = function () {                      
            ...
            //判断这个座位玩家是否有人
            if (seat[index] > 0) {
                //cc.log(people_num+'有人--'+common.card_time);
                //判断发牌3次3*people_num
                if (common.card_time < 3 * people_num) {
                    ...
                    //cc.log('--------'+common.card_number.length);
                }
            }
        ...
        if (index == 0 ){
            card_new.setScale(cc.v2(0.65,0.65))
        }
        else {
            card_new.setScale(cc.v2(0.45,0.45))
        }       
        ...
        //cc.log('第几次：'+common.card_time+'坐标：'+common.card_x);
        let move = cc.moveTo(time, pos.x + common.card_x, pos.y);   //移动代码
        let call = cc.callFunc(function () {
            //cc.log('递归：'+common.card_time);                     
            player_have()//递归回调           
        })
        ...
        //cc.log('发出去的牌');       
        //cc.log(common.card_number);       
        if (index == 0) {
            //cc.log('存储自己的牌' + common.card_number.length);
            common.card_number_isme.push(common.card_number.length);
            //cc.log('存储自己的牌---' + common.card_number_isme.length);
        }    
        common.card_game_end[index].push(card_new)
        //cc.log('存储自己的牌---' + index);
        //cc.log('存储自己的牌---' + common.card_game_end);
    }
    //收牌 牌动画 1所有的牌 2弃牌回收玩家座位号
    static card_recycle(begin_node,time) {
        let dis = begin_node.getPosition()
        //cc.log('手牌长度:'+common.card_number.length);        
    ...
    static card_draw(str) {
        ...
        //cc.log(common.card_number);      
        //反转1旋转动画
        let sc = cc.scaleTo(0.1, 0, 0.65)
        let call = cc.callFunc(function () {
            //cc.log('翻开牌:'+common.card_number[common.card_number_isme[common.draw_num] - 1]);            
            common.card_number[common.card_number_isme[common.draw_num] - 1].getComponent(cc.Sprite).spriteFrame = str[common.draw_num]
            //反转回来动画
            let sc = cc.scaleTo(0.1, 0.65, 0.65)
            let call = cc.callFunc(function () {
                common.draw_num += 1;
    ...
    static hint_move(node,selt,msg) {
        //点击提升框移动
        let move = cc.instantiate(node);
        if (msg == null){
            msg = '弃牌或等游戏结束后才可离开房间'
        }
        move.getChildByName('hint-text').getComponent(cc.Label).string = msg;
        selt.addChild(move,3);
    ...
    //     
    // }
    static intToFloatStr(num) {
        let _len = 5;
        let _str = num + '';
        let strArr = _str.split(".");
        if (strArr[0].length < _len && strArr[1]) {
            _str = strArr[0] + "." + strArr[1].substr(0, _len - strArr[0].length);
        }
        return _str;
    }
    // //金币显示转换
    static gold_transition(num,place) {
        num = Number(num);
        let _num = num / 100;
        let _zi = "";
        if (_num >= 100000000) {
        ...
        //         place = 0
        //     }
        //     let num1 = (num * 0.01).toFixed(place)
        //     return num1
        // }
        return this.intToFloatStr(_num) + _zi;
    ...
    //网络图片,头像专用
    static setSpriteWithUrl(node:cc.Node, url:string, size?:cc.Size){
        if(url.search("http") === -1) return;
        cc.loader.load(url, (err,text)=>{
